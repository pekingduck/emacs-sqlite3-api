  ...
  (let ((db-file (make-temp-file "sqlite3"))
    ...
    (unwind-protect
	 (progn
	   ...
	   (should (= sqlite-ok
		      (sqlite3-exec dbh "create table temp ( id integer primary key autoincrement )")))
	   (should (= sqlite-ok
...
(ert-deftest sqlite3-test-memory-db ()
  (let* ((dbh (sqlite3-open ":memory:" sqlite-open-create sqlite-open-readwrite))
	 stmt)
  ...
  (let (dbh stmt)
    ...
    (unwind-protect
	 (progn
	   ...
	   (should (= sqlite-ok
		      (sqlite3-exec dbh "create table test ( id integer primary key autoincrement )")))
	   (should (= sqlite-ok
    ...
    (unwind-protect
	 (progn
	   (setq dbh (sqlite3-open ":memory:"
				   sqlite-open-readwrite
				   sqlite-open-create))
	   (sqlite3-exec dbh "create table temp (name text, age integer, iq real, ting text)")
	   (setq stmt (sqlite3-prepare dbh "insert into temp values (?,?,?,?)"))
	   (cl-loop for i from 1 to times do
		(should (= sqlite-ok
			   (sqlite3-bind-multi stmt (format "name%d" i) i 123.4567 nil)))
		(sqlite3-step stmt)
		(sqlite3-reset stmt))
	   (sqlite3-finalize stmt)
